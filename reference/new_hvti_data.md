...
This is the single internal entry point used by every `hvti_*()`
function to create its return value. It enforces the three-slot contract
...
``` r
new_hvti_data(data, meta, tables = list(), subclass)
```
...
- subclass:
  ...
  A single string naming the specific subclass (e.g. `"hvti_survival"`).

## Value

A named list of class `c(subclass, "hvti_data")`.
