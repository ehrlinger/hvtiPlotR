...
- **`indentation_linter` and `commented_code_linter` are OFF**,
  deliberately: both fire heavily on the aligned-argument style used
  throughout `R/` and on vignette chunks that show an alternative call
  commented out. Everything else is lintr’s default and **is** enforced
...
- **A plot test must prove the plot has data.**
  ...
  `GeomAbline`, `GeomBlank`) whose layer data is constant decoration and
  therefore *not* evidence that the plotted dataset has rows.
  `expect_s3_class(plot(obj), "ggplot")` alone is a smoke test, not
  coverage.
