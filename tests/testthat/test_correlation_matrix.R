...
test_that("sample correlation groups agree with displayed boundary values", {
  d <- sample_correlation_data(n = 300, seed = 42)
  expect_true(any(d$a1c == 6))
  expect_true(any(d$a1c == 7))
  expect_true(all(d$a1c_grp[d$a1c %in% c(6, 7)] == "6-7"))
})
...
test_that("a two-observation panel is described as sparse, not empty", {
  d <- sample_correlation_data(n = 30)
  d$sparse <- c(1, 2, rep(NA_real_, 28))
  w <- expect_warning(hv_correlation_matrix(d, c("a1c", "sparse")))
  expect_match(w$message, "sparse")
  expect_false(grepl("empty or flat", w$message, fixed = TRUE))
})
...
test_that("disjoint variables warn about inadequate pairwise overlap", {
  d <- data.frame(
    a = c(1, 2, 3, NA, NA, NA),
    b = c(NA, NA, NA, 4, 5, 6)
  )
  expect_warning(hv_correlation_matrix(d, c("a", "b")), "a.*b")
})
...
test_that("plot validates alpha", {
  cm <- hv_correlation_matrix(sample_correlation_data(n = 20), c("a1c", "glucose"))
  expect_error(plot(cm, alpha = 1.1), "alpha")
})

test_that("plot retains an empty panel for an all-missing variable", {
  d <- sample_correlation_data(n = 20)
  d$all_na <- NA_real_
  cm <- suppressWarnings(
    hv_correlation_matrix(d, c("a1c", "glucose", "all_na"),
                          labels = c("HbA1c", "Glucose", "All missing"))
  )
  layout <- ggplot2::ggplot_build(plot(cm))$layout$layout
  strips <- unique(c(as.character(layout$row_var), as.character(layout$col_var)))
  expect_true("All missing" %in% strips)
})
