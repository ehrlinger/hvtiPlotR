...
#' (`hv_*()`) to validate and prepare data, then call [plot()] on the result
...
#'   [hv_theme_ppt()]: Large-font theme with a dark panel for
#'   PowerPoint slides.
#' * [hv_theme_light_ppt()] / [theme_light_ppt()]: Large-font theme with
...
#' ## Plot constructors
#'
#' Each constructor returns an `hv_data` S3 object; call [plot()] on it
#' to produce a ggplot.
#'
#' ### Propensity Score & Matching
#'
#' * [hv_mirror_hist()]: Prepare propensity-score distributions for a mirrored
...
#' * [hv_stacked()]: Prepare grouped count or proportion data for a stacked
#'   or filled histogram.
#'
#' ### Survival & Hazard
#'
#' * [hv_survival()]: Fit and prepare Kaplan-Meier or Nelson-Aalen survival
...
#'
#' * [hv_trends()]: Prepare patient-level data for annual trend lines with
...
#' * [hv_longitudinal()]: Prepare pre-aggregated counts for a grouped bar
...
#' * [hv_sankey()]: Prepare cluster-assignment data for a stability Sankey
...
#'   `Order_Variables()`.
#' * [hv_upset()]: Prepare binary indicator data for an UpSet intersection
