...
completed in SAS. In this case, the first step in creating figures in R
is to get the data out of SAS. There are many ways to do this, but we
have had success using the tp.bd.SAStoR.sas program (under the datasets
folder) to export a data set into a SAS xport file.

Once an xport file has been created, we read it into R with the foreign
package. This recipe reads an xport into a data.frame and then re-reads
the xport file to pull out the SAS labels to describe the variables. The
labels will be stored in a vector, which we can index by the variable
names.
...
installed using the `devtools::install_github("ehrlinger/hvtiPlotR")`
command.
