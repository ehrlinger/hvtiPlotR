...
template. We use a fictional template `tp.np.bmi.avrg_curv.binary.sas` as the
running example, and port it the way every new plot family in the package is
built (`hazard_plot()`, `survival_difference_plot()` and `nnt_plot()` are
legacy single-call functions kept for compatibility, not a pattern to copy): a
constructor, `hv_bmi_curve()`, that validates the data and returns an
`hv_data` object, and a `plot.hv_bmi_curve()` method that turns that object
