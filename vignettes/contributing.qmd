...
family lives in its own file, with the plot functions and their sample-data
companions together.
...
Add a bullet under the `# hvtiPlotR (unreleased)` heading at the top of `NEWS.md`,
adding that heading if it is not already there:
...
Run these three commands in order before pushing your branch. All three must
complete cleanly, with zero errors, zero warnings, and zero notes from
`check()`. Then open a pull request against `main` on GitHub.
