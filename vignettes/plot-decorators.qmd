...
numbers-at-risk panel. `survival_curve()` returns the risk table as a data
frame via `attr(result, "risk_table")` — columns `strata`, `report_time`,
`n.risk`. Build a ggplot text panel from it, then stack with `/`.
