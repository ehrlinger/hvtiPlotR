...
Once an xport file has been created, we read it into R with the
[haven](https://haven.tidyverse.org/) package. `haven::read_xpt()` reads
the file into a tibble and preserves SAS variable labels as the `"label"`
attribute on each column, so no second pass is required.

```r
library("haven")
# The xport file name
dtFilename <- system.file("extdata", "par_cst.xpt", package = "hvtiPlotR")
# Read the xport file into a tibble; SAS labels are stored as column attributes
dta <- haven::read_xpt(dtFilename)
# Extract labels into a named character vector for later use
dta.labels <- vapply(dta, function(col) {
  lbl <- attr(col, "label")
  if (is.null(lbl) || lbl == "") NA_character_ else lbl
}, character(1L))
# Replace NAs with the variable name so every entry has a fallback
dta.labels[is.na(dta.labels)] <- names(dta.labels)[is.na(dta.labels)]
```
...
PowerPoint file. Apply `hvti_theme(“dark_ppt”)` or `hvti_theme(“light_ppt”)`
before saving.
