...
This document outlines how to generate figures using the `ggplot2` package in **R**. Our approach is to demonstrate the **R** commands to generate the same elements created with `plot.sas` commands. [The `plot.sas` macro](#sec-plotsas) gives an overview of the methodology of the `plot.sas` macro and
...
|---|---|---|
| `tuple set=..., x=t, y=est, cll=lo, clu=hi` | `hv_nonparametric(curve_data, lower_col="lo", upper_col="hi")` | Preferred hvtiPlotR function |
