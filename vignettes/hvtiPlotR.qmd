...
datasets expected by **hvtiPlotR** plot functions (e.g., `nonparametric_curve_plot()`,
`hazard_plot()`).
...
- **Use `hvti_theme("dark_ppt")` or `hvti_theme("light_ppt")`** — the dark
...
- **Multi-group figures** — use `scale_colour_brewer(palette = "Set1")` for
  up to five groups. For more groups, select a palette manually or use
  `RColorBrewer::brewer.pal()` to inspect options.
