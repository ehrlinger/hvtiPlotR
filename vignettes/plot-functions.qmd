...
| `hv_survival_difference()` | `tp.hp.dead.life-gained.sas`, `tp.hp.numtreat.survdiff.matched.sas`, `tp.hs.dead.compare_benefit.setup.sas` | — |
| `nnt_plot()` | `tp.hp.numtreat.survdiff.matched.sas` | — |
| `hv_nnt()` | `tp.hp.numtreat.survdiff.matched.sas` | — |
...
| `hv_correlation_matrix()` | `descriptive/dc.tables.ods.sas` (`PROC CORR PLOTS=MATRIX`) | — |
| `hv_eda()` | — | — |
...
make the ±10 % threshold visually obvious.
...
they cross and fragment, K has grown past what the data supports. It ports the PAM cluster stability figure from the HVTI
...
directly -- no grouping argument needed for the two-group case. The dashed
horizontal line at zero makes the no-benefit baseline immediately visible; a
...
you spend time styling the plot -- worth doing when you're scripting
