...
constructor (`hvti_*()`) to validate and prepare data; this returns an S3
object of class `c("hvti_<concept>", "hvti_data")`. Then call `plot()` on that
...
|---|---|---|
| `hvti_mirror_hist()` | — | `tp.lp.mirror-histogram_SAVR-TF-TAVR.R`, `tp.lp.mirror_histo_before_after_wt.R` |
| `hvti_stacked()` | — | — |
| `hvti_balance()` | — | `tp.lp.propen.cov_balance.R` |
| `hvti_followup()` | `tp.dp.goodness_followup.*`, `tp.dp.goodness_event.*` | — |
| `hvti_survival()` | `tp.hp.dead.sas` (basic) | `tp.hp.dead.number_risk.R` |
...
| `hvti_nonparametric()` | `tp.np.*.avrg_curv.*`, `tp.np.*.u.trend.*`, `tp.np.*.double.*`, `tp.np.*.mult.*`, `tp.np.*.phases.*`, `tp.np.z0axdpo.*` | — |
| `hvti_ordinal()` | `tp.np.*.ordinal.*` | — |
| `hvti_eda()` | — | — |
| `hvti_spaghetti()` | — | `tp.dp.spaghetti.echo.R` |
| `hvti_trends()` | `tp.lp.trends.sas`, `tp.lp.trends.age.sas`, `tp.lp.trends.polytomous.sas`, `tp.rp.trends.sas` | `tp.dp.trends.R` |
| `hvti_longitudinal()` | `tp.dp.longitudinal_patients_measures.*` | — |
| `hvti_alluvial()` | — | — |
| `hvti_sankey()` | — | PAM cluster stability analysis |
| `hvti_upset()` | — | — |
...
A common figure in propensity-matched analyses is the mirrored histogram, which displays the propensity score distributions for two treatment groups before and after matching. The **hvtiPlotR** package provides the `hvti_mirror_hist()` constructor to prepare the data, and `plot()` to render the figure.
...
A common exploratory figure is the stacked histogram, which shows how the composition of a numeric variable changes over time or across another grouping dimension. The **hvtiPlotR** package provides the `hvti_stacked()` constructor and a `plot()` method to generate this figure.
...
**hvtiPlotR** package provides the `hvti_followup()` constructor and a `plot()`
...
The **hvtiPlotR** package provides `hvti_balance()` to prepare data and
...
`hvti_survival()` estimates the Kaplan-Meier product-limit survival function
...
The **hvtiPlotR** package provides `hvti_eda()` for exploratory data analysis
...
`plot()` on an `hvti_eda` object always returns a bare `ggplot`. Colour scales,
axis labels, annotations, and `hvti_theme()` are added by the caller.
...
`hvti_alluvial()` prepares an alluvial (Sankey-style) diagram using
...
`hvti_sankey()` prepares a Sankey diagram showing how patients flow between
...
`hvti_trends()` ports the pattern from five SAS/R templates:
...
`hvti_spaghetti()` ports the pattern from `tp.dp.spaghetti.echo.R`: one
...
`hvti_nonparametric()` prepares pre-computed average curves from a two-phase
...
`hvti_ordinal()` prepares pre-computed grade-specific probability curves from a
...
`hvti_longitudinal()` reproduces the two-panel layout from
...
`hvti_upset()` wraps `ComplexUpset::upset()` to visualise surgical procedure
