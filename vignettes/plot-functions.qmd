...
`plot.hv_upset()` returns a **ggplot** (or a small `patchwork` composite
when `set_size = TRUE`, the default, to include the set-size sidebar).
Themes apply via `+` like every other hvtiPlotR plot:
