...
<<introFigure, fig.height=5/4*3, fig.width=5, message=FALSE, warning=FALSE, echo=FALSE, fig.cap="Demonstration figure">>=
library(knitr)
library(ggplot2)
library(dplyr)
library(hviciPlotting)

data(parametric, package="hviciPlotting")
data(nonparametric, package="hviciPlotting")
...
ccf_plot+ coord_cartesian(xlim=c(-.1,5.2), ylim=c(-1,101))
...
The \code{plot.sas} macro code is closed by the ending \code{);} characters, and \proglang{SAS} is instructed to \code{run;} the code. Running combines building the figure by combining elements from \code{label}, \code{axis} and \code{tuple} statements and saving it into the file specified by the \code{gsasfile} variable. 
