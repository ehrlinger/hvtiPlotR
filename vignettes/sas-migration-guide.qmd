...
| `tp.dp.female_bicus_preAR_sankey.R` | dp | `alluvial_plot()` | [Alluvial](#dp-sankey) |
| PAM cluster stability analysis | dp | `cluster_sankey_plot()` | [Cluster stability Sankey](#dp-cluster-sankey) |
...
| `tp.rp.trends.sas` | rp | `trends_plot()` | [Trends over time](#dp-trends) |
| `tp.lp.trends.sas` | lp | `trends_plot()` | [Trends over time](#dp-trends) |
| `tp.lp.trends.age.sas` | lp | `trends_plot()` | [Trends over time](#dp-trends) |
| `tp.lp.trends.polytomous.sas` | lp | `trends_plot()` | [Trends over time](#dp-trends) |
...
| `tp.lp.mirror-histogram_SAVR-TF-TAVR.R` | lp | `mirror_histogram()` | [Mirror histogram — binary-match](#dp-mirror) |
...
returns a single bare ggplot for the selected `plot_type` (default
`"survival"`). All five plots matching the SAS `%kaplan` / `%nelsont`
macro output flags (`PLOTS`, `PLOTC`, `PLOTH`, `PLOTL`) plus tidy data frames
are attached as attributes (e.g., `attr(result, "km_data")`).
...
before calling `covariate_balance()`. Pass `var_levels` to control the
...
**R equivalent:** `cluster_sankey_plot()`

`cluster_sankey_plot()` draws a Sankey diagram showing how patients flow
between letter-labelled clusters as the number of clusters K increases from 2
to 9. Each column is one K; flow bands show assignment changes between
consecutive K values; node labels show cluster letter + patient count.
...
| `geom_sankey()` + `geom_sankey_label()` | `cluster_sankey_plot()` |
...
Both scripts are superseded by `mirror_histogram()`. The function returns a
bare ggplot object directly. Diagnostics are printed as a message and attached
as `attr(mhist, "diagnostics")`; working data as `attr(mhist, "data")`.
Compose scales, annotations, and theme with the usual `+` operator.
